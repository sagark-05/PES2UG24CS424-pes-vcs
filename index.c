...
int index_load(Index *index) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        int fields = sscanf(line, "%15s %64s %llu %u %511[^\n]",
                            mode_str, hex, &mtime, &size, path);
        if (fields != 5 || strlen(hex) != HASH_HEX_SIZE || path[0] == '\0') {
            fclose(f);
            return -1;
        }
        ...
        char *endptr;
        errno = 0;
        unsigned long mode = strtoul(mode_str, &endptr, 8);
        if (errno != 0 || *endptr != '\0') {
            fclose(f);
            return -1;
        }
...
int index_save(const Index *index) {
    if (!index || index->count < 0 || index->count > MAX_INDEX_ENTRIES) return -1;

    IndexEntry *sorted = malloc((size_t)index->count * sizeof(IndexEntry));
    if (index->count > 0 && !sorted) return -1;
    if (index->count > 0) {
...
int index_add(Index *index, const char *path) {
    ...
    if (!S_ISREG(st.st_mode) || st.st_size < 0 || st.st_size > UINT32_MAX) return -1;
